...
class Input:
    ...
    allowed_font_sizes = (10, 12, 14, 16, 18, 20, 24)

    def __init__(self, **options):
        """ Options: x, y, font, color, restricted, max_length, prompt """
        self.options = Config(options, ['x', '0'], ['y', '0'], ['font', 'pygame.font.Font(None, 32)'], ['color', '(0,0,0)'],
                              ['restricted', '\'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!"#$%&\\\'()*+,-./:;<=>?@[\]^_`{|}~\''],
                              ['max_length', '-1'], ['prompt', '\'\''], ['input_width', '100'], ['default_text', '\'\''])
        ...
        self.default_text = self.options.default_text

        self.rendered_prompt = self.font.render(self.prompt, 1, self.color)
        self.prompt_size = self.rendered_prompt.get_size()
        self.input_pad = self.font.render(' ', 1, self.color).get_width() / 2
        self.input_box = pygame.Rect(self.x + self.prompt_size[0], self.y, self.input_width + self.input_pad * 2, self.prompt_size[1])
        self.focus_area = pygame.Rect(self.x, self.y, self.input_width + self.input_pad * 2 + self.prompt_size[0], self.prompt_size[1])
        self.focused = False
        self.value = ''
        self.shifted = 0
        self.meta = 0 # current level of meta key press (can be >1 with eg 2 cmd keys)
        self.locked = False
    ...
    def draw(self, surface):
        ...
        if self.focused:
            ...
            pygame.draw.line(surface, (169, 169, 169), (self.x + self.prompt_size[0], self.y), (self.x + self.prompt_size[0] + self.input_width + self.input_pad * 2 - 2, self.y))

        if self.value == '' and not self.focused:
            text = self.font.render(self.default_text, 1, (180, 180, 180))
        else:
            text = self.font.render(self.value, 1, self.color)
        surface.blit(text, (self.x + self.prompt_size[0] + self.input_pad, self.y))
    ...
    def update(self, events):
        ...
        if not self.locked:
            for event in events:
                if event.type == MOUSEBUTTONDOWN:
                    if self.focus_area.collidepoint(event.pos):
                        self.focused = True
                    else:
                        self.focused = False
                elif event.type == KEYUP:
                    if event.key == K_LSHIFT or event.key == K_RSHIFT: self.shifted -= 1
                    elif event.key == K_LMETA or event.key == K_RMETA: self.meta -= 1
                elif event.type == KEYDOWN:
                    if event.key == K_LSHIFT or event.key == K_RSHIFT: self.shifted += 1
                    elif event.key == K_LMETA or event.key == K_RMETA: self.meta += 1
                    elif self.focused:
                        if event.key == K_BACKSPACE and self.meta != 1: self.value = self.value[:-1]
                        elif event.key == K_BACKSPACE: self.value = ''
        ...
        if len(self.value) > self.max_length and self.max_length >= 0: self.value = self.value[:self.max_length]
